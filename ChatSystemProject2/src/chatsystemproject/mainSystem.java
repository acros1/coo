...
public class mainSystem implements Runnable{
	...
	public void run() {
		...
		try {
			...
			clientAddr = sock.getInetAddress();
                        ArrayList<Session> sessionStarted = this.ClThread.getApplicationWindow().getSessionStarted();
				...
				for (User u  : clientList) {
					if (u.getAddr().equals(clientAddr)) {
						...
						ServerThread serverThread = new ServerThread(u, sock,new Session(this.ClThread.getMainUser(),u,this.ClThread).getSessionWindow(),this.ClThread);
	...
	public ServerThread getServer(User client,SessionWindow WindowSession) {
		try {
			...
			ServerThread st = new ServerThread(client, sock,WindowSession,this.ClThread);
